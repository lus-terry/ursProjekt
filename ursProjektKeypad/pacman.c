...
void pokaziBodove(int score) {
	lcd_clrscr();
	lcd_gotoxy(3, 1);
	lcd_puts("Vaši bodovi: ");
	printf(lcd_putc, "%02d", score);
}



void bodovi(int dots1[10], int pacy, int n, int score) {
	for (int i = 0; i < n; i++) {
		if (dots1[i] == pacy){
			red1[dots1[i]] = pacman;
			score++;
...
void smjer(char direction) {
	...
	*/
	...
	//score = bodovi(poljeLcd, posPacmanx, posPacmany, score);
